...
parser.add_argument('--dataset', type=str, default='CelebA', help='select dataset')

argparse_opt = parser.parse_args()


import easydict
args = easydict.EasyDict({
    'dataset': argparse_opt.dataset,   #'CelebA',#'lsun',
    # 'dataroot':'/data/dataset' if argparse_opt.dataset=='CelebA' else '/data/dataset/lsun',
    'dataroot':'/data/dataset/' + argparse_opt.dataset,
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    dataset = dset.LSUN(root=opt.dataroot, classes=['church_outdoor_train'],#classes, #download=True,
                        transform=transforms.Compose([
                            transforms.Resize(opt.imageSize),
                            transforms.CenterCrop(opt.imageSize),
                            transforms.ToTensor(),
...
device = torch.device("cuda:{}".format(argparse_opt.gpu_num) if opt.cuda else "cpu")
ngpu = int(opt.ngpu)
nz = int(opt.nz)
ngf = int(opt.ngf)
ndf = int(opt.ndf)
...
fixed_noise = torch.randn(opt.saveImageNum, nz, 1, 1, device=device)
real_label = 1
fake_label = 0
...
info_dir = '../catgan_example/inception_model_info'

if os.path.exists(info_dir + real_images_info_file_name) :
    print("Using exist inception model info from :", real_images_info_file_name)
    inception_model_score.load_real_images_info(info_dir + real_images_info_file_name)
else :
    ...
    inception_model_score.save_real_images_info(info_dir + real_images_info_file_name)
...
import wandb
wandb.init(project='GAN_with_diversity', config = opt)
config = wandb.config
